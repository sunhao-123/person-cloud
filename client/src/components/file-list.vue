        ...
        >
          ...
          >
            <div
              ...
              @dblclick="dialogPDFBox(item)"
              @mouseup="showPopup"
              v-if="judgType(item.type) === 'PDF'"
            >
          ...
          >
            <div
              ...
              @mouseup="showPopup"
              v-if="judgType(item.type) === 'folder'"
            >
              <div class="fileIcon outside">
                <img
                  class="icon"
  ...
  methods: {
    ...
    judgType(type) {
      if (type.toLowerCase() === ".folder") {
        ...
        type.toLowerCase() === ".ico"
      ) {
        return "img";
      } else if (
        type.toLowerCase() === ".txt" ||
        type.toLowerCase() === ".html" ||
        type.toLowerCase() === ".css" ||
        type.toLowerCase() === ".crt" ||
        type.toLowerCase() === ".pem" ||
        type.toLowerCase() === ".js" ||
        type.toLowerCase() === ".log" ||
        type.toLowerCase() === ".json" ||
        type.toLowerCase() === ".url" ||
        type.toLowerCase() === ".vue" ||
        type.toLowerCase() === ".xml"
      ) {
        return "TEXT";
    ...
    dialogTXTBox(item) {
      this.$alert(
        `<div style="width:570px;height:630px;overflow-y:auto;"><pre class="txt${item.id}" style="margin:315px auto;">加载中......</pre>`,
        `${item.file_name}`,
        {
          dangerouslyUseHTMLString: true,
      ...
      let src = `${this.hostUrl}/file/download/${item.id}/${item.hash_name}`;
      this.$http
        .get(src)
        .then((res) => {
          let txt = document.querySelector(`.txt${item.id}`);
          txt.style.margin = "0 auto";
          txt.innerHTML = `<xmp>${res.data}</xmp>`;
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    // dialogMovieBox(item) {
    //   this.$alert(
    //     `<video class="movie${item.id}" src="${this.hostUrl}/file/download/${item.id}/${item.hash_name}" style="margin:0 auto;width:1070px;height:600px;" controls="controls" autoplay=true;>您的浏览器不支持在线播放！请先下载该资源！</video>`,
    //     `${item.file_name}`,
    //     {
    //       dangerouslyUseHTMLString: true,
    //       showConfirmButton: false,
    ...
    //       callback: (action) => {
    //         if (action === "cancel") {
    //           document.querySelector(`.movie${item.id}`).pause();
    //         }
    //       },
    //     }
    //   );
    ...
    dialogPDFBox(item) {
      console.log(item)
      this.$alert(
        `<div class="dpfshow${item.id}" style="width:965px;height:630px;padding:0;"><iframe style="width:965px;height:630px;margin:0 auto;" src="${this.hostUrl}/pdf/web/viewer.html?file=${this.hostUrl}/file/download/${item.id}/${item.hash_name}"></iframe></div>`,
        `${item.file_name}`,
        {
          dangerouslyUseHTMLString: true,
          showConfirmButton: false,
    ...
    dialogMp3Box(item) {
      this.$alert(
        `<audio class="music${item.id}" src="${this.hostUrl}/file/download/${item.id}/${item.hash_name}" style="margin:0 auto;width:470px;" controls="controls" autoplay=true;>您的浏览器不支持在线播放！请先下载该资源！</audio>`,
        `${item.file_name}`,
        {
          dangerouslyUseHTMLString: true,
          showConfirmButton: false,
    ...
    getIconType(item) {
      ...
      let iconType = "";
      let test = /^aep$|^ai$|^avi$|^cdr$|^crt$|^css$|^dmg$|^doc$|^docx$|^dwg$|^eps$|^exe$|^folder$|^gif$|^html$|^ipa$|^iso$|^jpeg$|^jpg$|^js$|^json$|^log$|^mov$|^mp3$|^mp4$|^new$|^pdf$|^php$|^png$|^ppt$|^pptx$|^psd$|^rar$|^ttf$|^txt$|^url$|^vue$|^xls$|^xlsx$|^zip$|^bmp$|^raw$|^tiff$|^xml$/;
      if (test.test(type)) {
